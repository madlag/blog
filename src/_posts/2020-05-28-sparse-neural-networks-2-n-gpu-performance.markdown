...
![André Ampère](/assets/images/mdm/1*M1ky4jno_MmGg5eR3tgGyg.png)André Ampère, 1825 (from Wikipedia)So that’s the perfect pretext to make a large digression on **GPU architectures** and why knowing better about them may matter for your daily Machine Learning jobs.
...
![](/assets/images/mdm/1*0b-KUkO2e22UPpPKrRi9PQ.png)From [https://youtu.be/Knd-U-avG0c](https://youtu.be/Knd-U-avG0c?t=109)So if you could efficiently and cheaply remove heat from the chips, you could get higher frequencies, but only marginally, and it gets quickly impractical (water-cooling, you know, is *cool*, but [not when it leaks](https://www.avadirect.com/blog/leaking-liquid-cooler-whats-next/)…).
...
![](/assets/images/mdm/1*Fruof2lsq_v7hdrXZUhRjg.png)From <https://unsplash.com/photos/VEVfbQtyB8s>You can think about it as a **city** (the computing cores), and the suburban workers coming each morning in the city (the data). **The city is 2D, the highways are 1D,** and of course, you get some heavy traffic jams. So you add some new **lanes** on the **highways** (the width of the memory bus), but it’s always the **bottleneck**
